...
public class TranscriptHash {
    ...
    private final Map<ExtendedHandshakeType, byte[]> msgData = new ConcurrentHashMap<>();
    private final Map<ExtendedHandshakeType, byte[]> hashes = new ConcurrentHashMap<>();
    ...
    public void record(HandshakeMessage msg) {
        List<TlsConstants.HandshakeType> ambigousTypes = Arrays.asList(TlsConstants.HandshakeType.certificate,
    ...
    private void computeHash(ExtendedHandshakeType requestedType) {
        for (ExtendedHandshakeType type : hashedMessages) {
            if (msgData.containsKey(type)) {
                hashFunction.update(Objects.requireNonNull(msgData.get(type)));
            }
